...
void Picture::Save(const wxString& filename)
{
    ...
    mMachineDrawableLeft->Save(root, L"leftMachine");
    mMachineDrawableRight->Save(root, L"rightMachine");
...
void Picture::Load(const wxString& filename)
{
    ...
    mMachineDrawableLeft->Load(root, L"leftMachine");
    mMachineDrawableRight->Load(root, L"rightMachine");
...
/**
 * Getter for MachineDrawable
 * @param left if machine is on the left
 * @return MachineDrawable
 */
std::shared_ptr<MachineDrawable> Picture::GetMachineDrawable(bool left)
{
    if (left)
    {
        return mMachineDrawableLeft;
    }

    return mMachineDrawableRight;
}

/**
 * Setter for mMachineDrawable
 * @param machineDrawable machine drawable to set
 * @param left if machine is on the left
 */
void Picture::SetMachineDrawable(std::shared_ptr<MachineDrawable> machineDrawable, bool left)
{
    if (left)
    {
        mMachineDrawableLeft = machineDrawable;
    }

    mMachineDrawableRight = machineDrawable;
}

/**
 * Create a dialog box for machine start time
 * @param parent parent window of the dialog
 * @return a dialog box that can set start time
 */
MachineStartTimeDlg Picture::CreateDlg(wxWindow *parent)
{
    return MachineStartTimeDlg(parent, mMachineDrawableLeft, mMachineDrawableRight);
}
